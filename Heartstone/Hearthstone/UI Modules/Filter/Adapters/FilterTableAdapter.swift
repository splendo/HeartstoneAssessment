...
internal final class FilterTableAdapter: NSObject {
    fileprivate struct Section {
        fileprivate let filter: HearthstoneFilter
        ...
        fileprivate init(filter: HearthstoneFilter) {
            self.filter = filter
        ...
        fileprivate func indexAndItem(for item: HearthstoneFilter.Item) -> (Int, Item)? {
            items.enumerated().first { _, value -> Bool in value.item == item }
        }
    }
    
    fileprivate struct Item {
        fileprivate let item: HearthstoneFilter.Item
    ...
    internal let availableFilters: [HearthstoneFilter]
    internal private(set) var activeFilters: HearthstoneActiveFilters
    
    internal init(availableFilters: [HearthstoneFilter], activeFilters: HearthstoneActiveFilters) {
